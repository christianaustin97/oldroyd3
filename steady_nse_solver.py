...
def navier_stokes_ldc(meshsize, eta):
    ...
    # boundary data
    g_lid = Constant((1.0, 0.0)) #g.n=0 on the lid, or top wall
    ...
    bc_lid   = DirichletBC(W.sub(1), g_lid, lid) # driving lid
    ...
    bcs = [bc_lid, bc_walls, bc_press] #possibly get rid of pressure regulator if it breaks something in NN solve
...
def navier_stokes_bearing(meshsize, r, ecc, eta):
    """
        Solves flow around a journal/plain bearing
        
        Here, the larger bearing always has radius 1, center (0,0)
        The smaller bearing has radius r, center (0, -ecc)
        
    """
    if(r>=1 or r<=0 or ecc<0 or r+ecc>1):
    ...
    inner_circle = Circle(Point(0.0, -ecc), r, 128)
    ...
    class Inner(SubDomain):
        def inside(self, x, on_boundary):
            ...
            return (on_boundary and (radius < r*r+DOLFIN_EPS))
    ...
    g_inner = Expression(("(s*x[1]+ecc)/r", "-s*x[0]/r"), s=speed_inner, r=r, ecc=ecc, degree=1) 
    ...
    """
    u_test = Function(P)
    bc = DirichletBC(P, Constant(1.0), Outer())
    bc.apply(u_test.vector())
    plot(u_test, title = "outer boundary test")
    plt.show()
    
    u_test = Function(P)
    bc = DirichletBC(P, Constant(1.0), Inner())
    bc.apply(u_test.vector())
    plot(u_test, title = "inner boundary test")
    plt.show()
    
    u_test = Function(P)
    bc = DirichletBC(P, Constant(1.0), DomainBoundary())
    bc.apply(u_test.vector())
    plot(u_test, title = "total boundary test")
    plt.show()    
    
    u_test = Function(P)
    bc = DirichletBC(P, Constant(5.0), TopPoint(), 'pointwise')
    bc.apply(u_test.vector())
    plot(u_test, title = "pointmass boundary test")
    plt.show()
    """
